...
Through data cleaning and SQLite merging, each county’s COVID-19 data was matched with its median income and yearly population estimates. The analysis grouped counties into income tiers and examined both total and per capita cases and deaths from 2020 to 2022.
...
+ COVID-19 death rates per 1,000 people showed a similar trend:
    ...
    + High-income counties: 1.06 deaths per 1,000

These findings suggest a possible relationship between median household income and COVID-19 impact during the 2020–2022 period in Kentucky.

However, the project does not establish causation and does not account for factors like population density, healthcare access, educational attainment, or vaccination rates, which may have influenced outcomes.
...
| ----------                                | ------------------------------------- | 
|  `county`                                 | Name of the county in Kentucky: also the key for SQL joins           
|  `date`                                   | Date of the recorded Covid-19 data     
|  `cases`                                  | Total number of confirmed COVID-19 cases as of that date in the county
|  `deaths`                                 | Total number of confirmed COVID-19 deaths as of that date in the county 
|  `Median Household Income`                | 2020 median annual income for households in the county                         
|  `Income_Tier`                            | Categorized income group for the county: Low, Middle, or High Income
|  `Population_2020` to `Population_2022`   | Estimated population of each county for the respective year
|  `year`                                   | Extracted from the `date`, helps match each row to the correct year's population
|  `year_month`                             | Month and year format (YYYY-MM), used for time-based trend analysis
|  `population`                             | Population matched to the corresponding year for accurate case/death comparisons
|  `cases_per_1000`                         | Number of COVID-19 cases per 1,000 people in that county on that date
|  `deaths_per_1000`                        | Number of COVID-19 deaths per 1,000 people in that county on that date
